<?php
/**
 * Submit Rental Valuation Request for Love View Estate
 */
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($postcode) || empty($bedrooms) || empty($propertyType) || empty($fullName) || empty($email) || empty($phone) || empty($moveDate)) {
        ...
        header('Location: rental-valuation.php?error=1');
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        $subject = "New Rental Valuation Request";
        $message = "A new rental valuation request has been submitted:\n\n";
        ...
        // Redirect with error
        header('Location: rental-valuation.php?error=2');
        exit;
    }
} else {
